...
(function () {
  ...
  function eligibleFor(dutyKey, members) {
    ...
    /*
     * 洗碗分兩群人，混在同一張圖裡看不出東西：
     * 招員早、晚固定洗（一天穩定 2 次），其他人是輪的（一天 0~1 次）。
     * 圖上只畫「在輪的那群」，固定的那五位次數照樣記，但不進來當分母，
     * 否則他們永遠是深橘色、其他人永遠是深藍色，圖就沒有意義了。
     */
    if (dutyKey === "dishwash") {
      return members.filter((m) => m.fixedRole !== "delivery" && !St.hasFixedDishwash(m));
    }
    ...
    /*
     * 撤收全員都排，所以分母是全員：送便當的兩位只排得到晚上、招員只排得到中午，
     * 但他們都真的在排，而且跟大家一樣一天最多一次，放在同一張圖裡比才有意義。
     * 只有「三餐都固定洗碗」的人整天排不到撤收，那種才要拿掉。
     */
    if (dutyKey === "cleanup") return members.filter((m) => !St.isFixedDishwashAllDay(m));
    /*
     * 打菜三項：招員現在跟大家一起輪，所以留在圖上。
     * 只有名冊上有打菜固定角色的人（打飯／計數／抬飲料）本來就不進這個輪替。
     */
    if (["serveDish", "lid", "boxing"].indexOf(dutyKey) !== -1) {
      return members.filter((m) => !m.servingRole);
    }
    /*
     * 剩下的是廚餘／擦桌子／清地板。
     *   固定送便當的兩位不排（他們在跑便當）。
     *   招員也不進這三張圖：廚餘他們中午固定做（一天穩定 1 次，不是在輪），
     *   擦桌子與清地板則完全不做——三張圖的 0 次或滿次都不是「做太多／太少」，
     *   算進來只會讓其他人全部被標歪。
     */
    if (dutyKey === "foodwaste") {
      return members.filter(
        (m) => m.fixedRole !== "delivery" && !St.hasFixedFoodwaste(m) && !St.hasFixedDishwash(m)
      );
    }
    return members.filter((m) => m.fixedRole !== "delivery" && !St.hasFixedDishwash(m));
  }
